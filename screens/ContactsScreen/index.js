    ...
    Contacts.getAllWithoutPhotos((err, contacts_) => {
      if (err === 'denied'){
        ...
        if (deviceContacts.length > 0) {
            const shuffleContact = shuffle(deviceContacts.map(c => c.phoneNumbers[0].number))
      ...
      <>
        <SafeAreaView>
            <View style={styles.body}>
                <ContactsList>
                    {contacts.map((c, i)=> <Contact key={i} title={c.name ? c.name : c.number} onPress={() => navigation.navigate('', { headerTitle: c.title })} />)}
                </ContactsList>
                <AddButton>
                    <Icon type="ADD" />
                </AddButton>
